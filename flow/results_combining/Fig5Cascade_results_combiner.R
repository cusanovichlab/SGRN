...
all = rbind(rep1,rep2)
all = rbind(all,rep3)
all = separate(all,exp_name,into=c("Enzyme","Activator","Guide","Target1","Target2"),remove=F)
...
all[grep("UT|PhiYFP|mCerulean|mCherry",all$exp_name),]$Target1 = "None"
all[grep("UT|PhiYFP|mCerulean|mCherry",all$exp_name),]$Target2 = "None"
all[grep("only",all$exp_name),]$Target2 = "P2"
all[grep("P2-only",all$exp_name),]$Target1 = "None"
all[grep("28nt|DR",all$exp_name),]$Target1 = "P1"
...
all_onechannel = select(all,exp_name:Target2,Replicate,Channel:MFI)
