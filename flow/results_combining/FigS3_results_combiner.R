...
all = rbind(rep1,rep2)
all = rbind(all,rep3)
all = separate(all,exp_name,into=c("Enzyme","Guide","Promoter"),remove=F)
...
all[grep("UT|only|PhiYFP|mCerulean|mCherry",all$exp_name),]$Guide = "None"
all[grep("UT|PhiYFP|mCerulean|mCherry",all$exp_name),]$Promoter = "None"
all[grep("P1-only",all$exp_name),]$Promoter = "P1"
all[grep("P2-only",all$exp_name),]$Promoter = "P2"
all[grep("P3-only",all$exp_name),]$Promoter = "P3"
write.table(all,paste0(figure,"_combined_table.txt"),row.names=F,quote=F,sep="\t")

all$Channel = NA
all$Channel[grep("P1|PhiYFP",all$exp_name)] = "PhiYFP"
all$Channel[grep("P2|mCerulean",all$exp_name)] = "mCerulean"
all$Channel[grep("P3|mCherry",all$exp_name)] = "mCherry"
uts = filter(all,exp_name == "UT")
all = rbind(all,uts)
all = rbind(all,uts)
all[all$exp_name == "UT" & all$Replicate == "Rep1",]$Channel = c("PhiYFP","mCerulean","mCherry")
all[all$exp_name == "UT" & all$Replicate == "Rep2",]$Channel = c("PhiYFP","mCerulean","mCherry")
all[all$exp_name == "UT" & all$Replicate == "Rep3",]$Channel = c("PhiYFP","mCerulean","mCherry")
...
all_onechannel = select(all,exp_name:Promoter,Replicate,Channel,PercPos:MFI)
