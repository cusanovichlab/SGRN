...
all = rbind(rep1,rep2)
all = rbind(all,rep3)
all = separate(all,exp_name,into=c("Enzyme","Target","TargetN"),remove=F)
...
all[grep("UT|PhiYFP|mCerulean|mCherry",all$exp_name),]$TargetN = "0xTS"
all[grep("UT|PhiYFP|mCerulean|mCherry",all$exp_name),]$Target = "None"
all[grep("only",all$exp_name),]$TargetN = all[grep("only",all$exp_name),]$Target
all[grep("Original|only",all$exp_name),]$Target = "P2"
all[grep("Original",all$exp_name),]$TargetN = "7xTS"
...
all_onechannel = select(all,exp_name:TargetN,Replicate,Channel,PercPos:MFI)
