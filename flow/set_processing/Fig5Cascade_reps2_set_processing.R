...
trLogicle4=logicletGml2(parameters = "Pacific Orange-A", T = 262144, M = 4.5, 
                        W = 0.5, A = 0, transformationId="trLogicle4")
...
logicler = function(x){
  ...
  logicle_PacificOrange_A  = eval(trLogicle4)(exprs(x)[,7])
  new_logicles = cbind(logicle_FITC_A,logicle_AmCyan_A)
  new_logicles = cbind(new_logicles,logicle_PE_Cy5_A)
  new_logicles = cbind(new_logicles,logicle_PacificOrange_A)
  colnames(new_logicles) = c("logicle-FITC-A","logicle-AmCyan-A","logicle-PE-Cy5-A","logicle-Pacific-Orange-A")
...
pacorange_gate = rectangleGate(filterId = "pacorange",list("logicle-Pacific-Orange-A"=c(0.21,1),"SSC-A"=c(5000,215000)))
p8 = ggcyto(singlets, aes(x = `logicle-Pacific-Orange-A`, y =  `SSC-A`)) + scale_y_continuous(labels = scientific)
p8 = p8 + geom_hex(bins = 256) + theme_minimal() + scale_fill_viridis(trans="log10")
p8 = p8 + geom_gate(pacorange_gate) + geom_stats(size = 3)
p9 = ggcyto(singlets, aes(x = `logicle-Pacific-Orange-A`, fill = factor(name))) + geom_density(alpha = 0.2) + theme_minimal()
...
pacorange_positives = Subset(singlets,pacorange_gate)
...
num_pacorange = fsApply(pacorange_positives,function(x) nrow(exprs(x)))
...
pacorange_MFI = fsApply(pacorange_positives,function(x) median(exprs(x)[,7]))
...
results = data.frame("exp_name" = shortnames,"exp_date" = fullset[[1]]@description$`$DATE`,"num_events" = num_events,
                     ...
                     "num_mcerulean" = num_mcerulean, "num_mcherry" = num_mcherry,"num_pacorange" = num_pacorange, "phiyfp_MFI" = round(phiyfp_MFI,1),
                     "mcerulean_MFI" = round(mcerulean_MFI,1), "mcherry_MFI" = round(mcherry_MFI,1), "pacorange_MFI" = round(pacorange_MFI,1))
...
tr4Pars=list(transformReference("trLogicle4",flowEnv))
...
pacorange_gate@parameters=new("parameters",tr4Pars)
flowEnv[['pacorange_gate']]=pacorange_gate
